...
def fit_transformer(
    ...
    english_tokenizer = BertTokenizer.from_pretrained(
        "bert-base-uncased"
    )  # Utilisez le tokenizer BERT
    french_tokenizer = BertTokenizer.from_pretrained(
        "dbmdz/bert-base-french-europeana-cased"
    )
    # Tokenize, encode, and pad source sentences
    src_tokens = [
        french_tokenizer.encode(
            text,
    ...
    for epoch in range(num_epochs):
        ...
        for index, (src_batch, trg_batch) in enumerate(train_loader):
            ...
            trg_batch = F.one_hot(
                trg_batch, num_classes=model.target_vocab_size
            ).float()
            loss = criterion(
                outputs.view(-1, model.target_vocab_size),
                trg_batch.view(-1, model.target_vocab_size),
            )
...
def get_translation():
    ...
    src_tokenized = french_tokenizer.encode(
        french_sentence,
        add_special_tokens=True,
        max_length=15,
        pad_to_max_length=True,
    ...
    src_tensor = torch.tensor(src_tokenized, dtype=torch.long).unsqueeze(
        0
    )  # Ajouter une dimension pour le lot (batch)

    trg_tokenized = english_tokenizer.encode(
        "Hello ! How are you ?",
        add_special_tokens=True,
        max_length=15,
        pad_to_max_length=True,
    ...
    trg_tensor = torch.tensor(trg_tokenized, dtype=torch.long).unsqueeze(
        0
    )  # Ajouter une dimension pour le lot (batch)

    # Utiliser le modèle pour effectuer la traduction
    with torch.no_grad():
        translation_ids = model.decode(
            src_tensor, trg=trg_tensor
        )  # trg=None car nous n'avons pas besoin de la cible pour la traduction

    # Décoder les identifiants de tokens en phrases en anglais
    english_translation = english_tokenizer.decode(
